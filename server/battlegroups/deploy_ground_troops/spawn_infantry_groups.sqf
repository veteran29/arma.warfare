AW_spawn_random_infantry_group = {
	...
	private _group = [_pos, _side, _can_spawn] call AW_spawn_squad;
	[_group] call add_battle_group;
};

AW_get_infantry_spawn_position = {
	params ["_pos", "_side"];
	...
	private _preferred_target = [_target_sectors, _pos] call AW_find_closest_sector;
...
AW_spawn_squad = {
	...
	_pos = [_pos, _side] call AW_get_infantry_spawn_position;
	_soldier_count = (squad_cap call AW_calc_number_of_soldiers) min _can_spawn;
    [_pos, _side, _soldier_count, false] call AW_spawn_infantry;	
};

AW_spawn_infantry_groups = {
	params ["_side"];
	...
	if (_can_spawn > (squad_cap / 2) || (_strength == _can_spawn)) exitWith {
		[_side, _can_spawn] call AW_spawn_random_infantry_group;		
	};	
};