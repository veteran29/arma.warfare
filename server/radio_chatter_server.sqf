...
AW_report_casualities_over_radio = {
	params ["_group"];
	...
	if (_deaths > 0) exitWith {
		...
		if (_distance > 200) exitWith {
			...
			[_group, _deaths, _distance, _direction, _location] remoteExec ["AW_report_casualities"];
		};

		[_group, _deaths, _location] remoteExec ["AW_report_casualities_in_sector"];
	};
};
...
report_next_waypoint = {
	...
	if (_count > 0) then {
		...
		[_group, _msg] remoteExec ["AW_group_report_client"];
	};
};

HQ_report = {
	params ["_side", "_msg"];
	[_side, _msg] remoteExec ["AW_HQ_report_client"];
};
