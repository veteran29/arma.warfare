AW_add_take_manpower_from_player_action = {  
  player addAction [["Take manpower", 0] call AW_addActionText, {
    ...
    }, nil, 80, true, true, "", '[cursorTarget] call AW_can_take_manpower_from_player'];
}; 

AW_add_give_manpower_to_player_action = {  
  player addAction [["Give manpower", 0] call AW_addActionText, {
		  params ["_target", "_caller"];
    ...
    }, nil, 80, true, true, "", '[cursorTarget] call AW_can_give_manpower_to_player'];
};

AW_can_take_manpower_from_player = {
...
AW_can_give_manpower_to_player = {
  params ["_other_player"];
...
AW_add_manpower_action = {
	params ["_box"];

	_box addAction [["Add manpower points", 0] call AW_addActionText, {	
		params ["_target", "_caller"];
  ...
  '[_target, _this] call owned_box && [_this] call AW_player_has_manpower', 10
  ];	
};

AW_add_take_manpower_action = {  
  params ["_box"];

  _box addAction [["Take manpower", 0] call AW_addActionText, {
      params ["_target", "_caller"];
    ...
    '[_target] call AW_box_has_manpower', 10
    ];
};

AW_add_store_manpower_action = {
  params ["_box"];

  _box addAction [["Store manpower", 0] call AW_addActionText, {
      params ["_target", "_caller"];
  ...
  '[_this] call AW_player_has_manpower',10
  ];
};

AW_player_has_manpower = {
    params ["_player"];
