AW_remove_vehicle_action = {
	params["_veh"];

  	_veh addAction [["Return vehicle", 0] call AW_addActionText, {
    	params ["_target", "_caller"];

		if(0 == count crew _target) exitWith {
			deleteVehicle _target;
			systemChat "Vehicle returned";
		};
		
		systemChat "Vehicle is not empty";
  	}, nil, 70, false, true, "",
  	'[_this] call AW_is_player_in_hq && [_target, _this] call AW_is_same_side && [_this] call AW_not_in_vehicle', 10
  	];
};

AW_is_same_side = {
	params ["_vehicle","_player"];
	(side _vehicle) isEqualTo civilian || (side _vehicle) isEqualTo (side _player);
};

AW_is_player_in_hq = {
	params ["_player"];

  	private _side = side _player;
  	private _respawn_marker = [_side, respawn_ground] call AW_get_prefixed_name;
	private _pos = getMarkerPos _respawn_marker;

	(getPos _player) distance _pos < 50; 
};
